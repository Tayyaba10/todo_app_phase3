...
export default function ChatPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiClient.post(`/api/${user.id}/chat`, {
        message: inputValue,
      });

      const { response: agentResponse, conversation_id } = response.data;
      ...
      <div className="flex-1 overflow-y-auto p-4 bg-gray-50">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <h2 className="text-lg font-semibold mb-2">Welcome to the AI Todo Assistant!</h2>
